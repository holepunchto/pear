...
test('Pear.updates should notify App stage updates (different pear instances)', async function (t) {
  ...
  comment('staging app')
  const appStaging = appStager.stage({ channel: `test-${fid}`, name: `test-${fid}`, dir: dir, dryRun: false, bare: true })
  ...
  const appSeeding = appSeeder.seed({ channel: `test-${fid}`, name: `test-${fid}`, dir: dir, key: null, cmdArgs: [] })
  ...
  const appStaging2 = appStager2.stage({ channel: `test-${fid}`, name: `test-${fid}`, dir: dir, dryRun: false, bare: true })
