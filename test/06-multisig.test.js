...
try {
  ...
  constants.init('dev', false, null)
}

test('pear multisig key generation confirms password', async function ({
  alike,
  absent,
  is,
  ok,
  ...
  const answers = ['password', 'not-password']
  readPassword = (prompt = 'Password: ') => {
    mismatchPrompts.push(prompt)
    return answers.shift()
  }

  let error = null
  try {
    await run(command('mismatch'))
  } catch (err) {
    error = err
  }
  ...
  is(error.message, 'Passwords do not match')
  alike(mismatchPrompts, ['Password: ', 'Confirm password: '])
  absent(fs.existsSync(path.join(sign, 'mismatch.public')))
  absent(fs.existsSync(path.join(sign, 'mismatch')))
  ...
  await run(command('matching'))

  alike(matchingPrompts, ['Password: ', 'Confirm password: '])
  ok(fs.existsSync(path.join(sign, 'matching.public')))
  ok(fs.existsSync(path.join(sign, 'matching')))
})

function command(name) {
  return {
    command: { name: 'get', parent: { name: 'keys' }, flags: { json: false } },
    args: { name },
    flags: { secret: false }
  }
}
