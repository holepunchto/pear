...
module.exports = class Pre extends Readable {
  ...
  async #pre () {
    ...
    const topPre = this.options.pre ? (Array.isArray(this.options.pre) ? this.options.pre : [this.options.pre]) : []
    ...
    const hasPipe = isWindows ? fs.fstatSync(3).isFIFO() : fs.fstatSync(3).isSocket()
    if (hasPipe) {
      this.#final()
      return
    }
  ...
  #run (options, link, index, from) {
    ...
    sp.stdio[1].on('data', (output) => this.push({ tag: 'preio', data: { from, output, index, fd: 1 } }))
    sp.stdio[2].on('data', (output) => this.push({ tag: 'preio', data: { from, output, index, fd: 2 } }))
    const pipe = sp.stdio[3]
    this.once('end', () => {
      pipe.end()
      pipe.destroy()
      try { sp.kill() } catch { /* ignore if already closed */ }
    })
        ...
        if (output.tag === 'configure') {
          ...
          pipe.on('data', () => {
            try {
              ...
              this.push({ tag: 'pre', data: { from, output: err, index, success: false } })
            }
          })
          resolve(output.data)
        }
