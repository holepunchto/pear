...
class DriveMonitor extends Readable {
  constructor(drive, options = {}) {
    ...
    this.drive = drive
    this.speedometers = {
      download: new Speedometer(),
      upload: new Speedometer()
    }
    this.peers = 0
  ...
  reading(delta) {
    ...
    while (dist--) {
      ...
      this.buffer[this.pointer] =
        this.buffer[this.pointer === 0 ? this.size - 1 : this.pointer - 1]
      this.pointer++
    }
    ...
    const btm =
      this.buffer.length < this.size
        ? 0
        : this.buffer[this.pointer === this.size ? 0 : this.pointer]

    return this.buffer.length < this.resolution
      ? top
      : ((top - btm) * this.resolution) / this.buffer.length
  }
