...
class Sidecar extends ReadyResource {
  ...
  constructor ({ updater, drive, corestore, nodes, gunk }) {
    ...
    this.App = class App {
      ...
      report (report) {
        console.log('MAKING A REPORTRRTT', report)
  ...
  reports (params, client) {
    ...
    if (app === null && params.id) {
      const stream = new streamx.PassThrough()
      const rpt = this.sidecar.bus.sub({ topic: 'reports', id: params.id })
      streamx.pipeline(rpt, pickData(), stream)
      return stream
    }
    if (app instanceof this.App === false) return
    const stream = new streamx.PassThrough()
    console.log(rpt)
    rpt.on('close', () => { console.trace('HUHHHHH')})
    const rpt = this.sidecar.bus.sub({ topic: 'reports', id: app.id })
    streamx.pipeline(rpt, pickData(), stream)
    return stream
  }
