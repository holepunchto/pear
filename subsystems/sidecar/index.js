...
class Sidecar extends ReadyResource {
  ...
  wakeup (params = {}) {
    ...
    return new Promise((resolve) => {
      ...
      const matches = [...this.apps].filter((app) => {
        if (!app || !app.state) return false // TODO: probably safe to remove this line, check & remove
  ...
  async #start (flags, client, session, env, cwd, link, dir, startId, args, cmdArgs) {
    ...
    link = pearLink.normalize((link.startsWith('pear:') || link.startsWith('file:')) ? link : pathToFileURL(link).href)
