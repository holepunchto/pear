...
if (Bare.Thread.self && Bare.Thread.self.data._vanityThread) runThread()

module.exports = function findVanityKey(start) {
  const numThreads = (os.cpus() && os.cpus().length) || 1
  ...
  return new Promise((resolve) => {
    ...
    const stopAll = () => {
      for (const thread of runningThreads) thread.terminate()
    }
