...
module.exports = class Bundle {
  ...
  constructor (opts = {}) {
    const {
      ...
      updateNotify, updatesDiff = false
    } = opts
  ...
  async #updates (updateNotify) {
    ...
    try {
      if (this.updatesDiff) {
        ...
        for await (const diff of this.watchingUpdates) {
          const local = this.hexKey === null
          const length =  local ? 0 : this.drive.version
          const fork = local ? 0 : this.drive.core.fork
          updateNotify(
            { key: this.hexKey, length, fork },
            { link: this.link, diff }
          )
        }
      } else {
