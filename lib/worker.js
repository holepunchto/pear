...
class Worker {
  ...
  constructor ({ ref = noop, unref = noop, ipc = null, parent = null, parentVersion = null } = {}) {
    this.#ref = ref
    this.#unref = unref
    this.#ipc = ipc
    this.#parent = parent
  ...
  #args (link) {
    ...
    const { drive } = pearLink(constants.ALIASES)(link)
    const key = drive.key ? hypercoreid.encode(drive.key) : null
    const parentVersion = global.Pear?.config.version || this.#parentVersion // in terminal app, global.Pear is defined but in electron main process parent version has to be passed
    if (key && key === parentVersion.key) {
      args.splice(1, 0, '--checkout', parentVersion.length)
    }
    return args
  }
