...
class Worker {
  ...
  #args (link) {
    ...
    const cmdSliced = sliced.slice(cmdIdx + 1)
    const cmdBase = parser.parse(['run', ...cmdSliced])
    const cmd = cmdBase.flags.trusted ? cmdBase : parser.parse(['run', '--trusted', ...cmdSliced])
