...
class Worker {
  ...
  #args (link, opts = {}) {
    ...
    const argv = ['run', '--trusted']
    if (opts.encryptionKey) {
      argv.push('--encryption-key', opts.encryptionKey)
    }
    argv.push(...program.argv.slice(2))
  ...
  run (link, args = [], opts = {}) {
    ...
    args = [...this.#args(link, opts), ...args]
    const runtime = opts.runtime || constants.RUNTIME
    const sp = spawn(runtime, args, {
