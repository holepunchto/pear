...
class Worker {
  ...
  #args (link, encryptionKey) {
    const parser = command('pear', command('run', ...rundef))
    const sliced = program.argv.slice(1)
    const cmdIdx = shell(sliced).indices.args.cmd
    const encryptionKeyFlag = encryptionKey ? ['--encryption-key', encryptionKey] : []
    const argv = ['run', '--trusted', ...encryptionKeyFlag, ...sliced.slice(cmdIdx + 1)]
  ...
  run (link, args = [], encryptionKey) {
    if (isElectronRenderer) return this.#ipc.workerRun(link, args)
    args = [...this.#args(link, encryptionKey), ...args]
