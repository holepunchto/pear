...
class Worker {
  ...
  constructor ({ ref = noop, unref = noop, ipc = null, parent = null } = {}) {
    this.#ref = ref
    this.#unref = unref
    this.#ipc = ipc
    this.#parent = parent
  ...
  run (link, args = []) {
    if (isElectronRenderer || isBare) {
      const { key: workerKey, fork: workerFork, length: workerLength } = (pearLink(constants.ALIASES)(link)).drive
      const { key: appKey } = (pearLink(constants.ALIASES)(Pear.config.applink)).drive
      if (appKey && b4a.equals(workerKey, appKey) && !workerFork && !workerLength) {
        link = this._rewriteLink(link, Pear.config.length, Pear.config.fork)
      }
    }
  ...
  _rewriteLink (link, length, fork) {
    const { drive, pathname } = (pearLink(constants.ALIASES)(link))
    return `pear://${fork}.${length}.${hypercoreid.encode(drive.key)}${pathname || ''}`
  }
}
