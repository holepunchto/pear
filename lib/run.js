...
const { APPLING_DOES_NOT_EXIST, PERMISSION_REQUIRED } = require('./errors')
  ...
  try {
    if (detached) {
      ...
      try {
        ...
        ipc.close().catch(console.error)
        throw APPLING_DOES_NOT_EXIST('Appling does not exist')
      }
    ...
    if (bail && args.indexOf('--detach') === -1) {
      const err = PERMISSION_REQUIRED('Permission required to run key')
      err.key = key
      throw err
    }
