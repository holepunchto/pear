...
const fsp = isBare ? require('fs/promises') : require('bare-fs/promises')
...
module.exports = async function run ({ rpc, args, key, storage, detached, dir }) {
  ...
  try {
    ...
    const { startId, host, id, type, bundle } = await rpc.start({ argv: args, env: ENV, cwd })
  ...
  child.once('exit', (code) => { stream.push({ tag: 'exit', data: { code } }) })
