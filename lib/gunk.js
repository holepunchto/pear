...
const builtins = {
  has (ns) { return overrides.includes(ns) || defaults.builtins.has(ns) },
  get (ns) {
    switch (ns) {
      ...
      default: return defaults.builtins.get(ns)
    }
  },
  keys () { return [...defaults.builtins.keys(), ...overrides] }
}
...
const mapImport = function (id) {
  ...
  // TODO: holepunch:// protocol is legacy, keet must move to pear/ builtin entirely before legacy can be dropped
