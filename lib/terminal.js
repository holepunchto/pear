...
function print(message, success) {
  if (statusFrag) stdio.out.write('\x1B[2K\r') // clear line and return cursor
  statusFrag = ''
  ...
  (opts, stream, info = {}) => {
    ...
    const promise = opwait(stream, ({ tag, data }) => {
      ...
      const transform = Promise.resolve(
        typeof taggers[tag] === 'function' ? taggers[tag](data, info) : taggers[tag] || false
      )
      transform.then(
        (result) => {
...
class TerminalTableRenderer {
  constructor({ json, tty, untilSync }) {
    this.json = json

    const { width } = stdio.size()
    this.ctrlTTY = !this.json && tty !== false && isTTY && !!width
    this.listenForCtrlC = tty === false && isTTY && !untilSync
  ...
  setResizeHandler() {
    if (this.ctrlTTY) {
      stdio.out.off('resize', this.render)
      stdio.out.on('resize', this.render)
    }
  }

  render() {
    if (this.json || !this.table) return
    if (!this.renderedTable) {
      this.setupTTYInput()
      this.setResizeHandler()
    }
    this.table.print(stdio, { clearScrollback: !this.renderedTable })
  ...
  setupTTYInput() {
    if (this.ctrlTTY) {
      ...
      stdio.in?.on('data', (key) => {
        ...
        const selectedTable = this.table.selectedTable
        if (selectedTable) {
          if (key.toString() === '\u001b[A') {
            selectedTable.up()
            ...
            this.table.selectNextScrollable()
          }
        }

        this.render()
      })
    } else if (this.listenForCtrlC) {
      stdio.in?.setMode?.(tty.constants.MODE_RAW)
      stdio.in?.on('data', (key) => {
