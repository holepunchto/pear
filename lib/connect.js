...
const { SOCKET_PATH, RUNTIME, PLATFORM_DIR, CONNECT_TIMEOUT } = require('./constants.js')

const { spawn } = whichRuntime.isBare ? require('bare-subprocess') : require('child_process')
const Pipe = whichRuntime.isBare ? require('bare-pipe') : require('net')

module.exports = bootpipe

function connect () {
  if (whichRuntime.isBare) return new Pipe(SOCKET_PATH)
