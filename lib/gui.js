...
const { FORCE_SHOW, ALIASES } = require('./constants')
...
class Menu {
  ...
  static WINDOW = 3
  template = [
    whichRuntime.isMac ? { label: 'Pear' } : { label: 'App' },
    {
      label: 'Edit',
      submenu: [
        { role: 'undo' },
        ...
        { role: 'copy' },
        { role: 'paste' },
        whichRuntime.isMac && { role: 'pasteAndMatchStyle' },
        { role: 'delete' },
        { role: 'selectAll' }
      ].filter(Boolean)
    },
  ...
  #app () {
    ...
    item.submenu.unshift(...[
      ...
      },
      {
        ...
        accelerator: whichRuntime.isMac ? 'Alt+Command+U' : 'Ctrl+Shift+U',
        visible: false,
        click (_, win) {
  ...
  #dev () {
    ...
    const dts = (electron.webContents.getAllWebContents() || [])
      ...
      .map((wc, ix) => {
        ...
        const item = {
          ...
          accelerator: (whichRuntime.isMac ? `Alt+Command+Shift+${ch}` : `Ctrl+Alt+Shift+${ch}`),
          click () {
            if (wc.isDestroyed()) {
              self.render()
              return
    ...
    item.submenu = [
      ...
      },
      {
        ...
        accelerator: whichRuntime.isMac ? 'Alt+Command+I' : 'Ctrl+Shift+I',
        visible: false,
        click (_, win) {
          if (!win) return
          const view = win.getBrowserViews()[0]
...
class Window extends GuiCtrl {
  ...
  async open (opts = {}) {
    ...
    this.win = new BrowserWindow({
      ...
      height,
      width,
      frame: !(whichRuntime.isMac || whichRuntime.isWindows),
      ...(whichRuntime.isMac && this.ctx.options.platform?.__legacyTitlebar ? { titleBarStyle: 'hidden', trafficLightPosition: { x: 12, y: 16 }, titleBarOverlay: true } : (whichRuntime.isMac ? { titleBarStyle: 'hidden', trafficLightPosition: { x: 0, y: 0 }, titleBarOverlay: true } : {})),
      ...(whichRuntime.isMac && this.ctx?.alias === 'keet' && this.ctx?.appling?.path ? { icon: path.join(path.dirname(this.ctx.appling.path), 'resources', 'app', 'icon.ico') } : {}),
      show,
    ...
    if (whichRuntime.isMac === false) this.win.setAutoHideMenuBar(true)
