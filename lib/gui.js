...
const { TESTING, NETWORK_RECORD, FORCE_SHOW, BOOT, ALIASES, IS_MAC, IS_WINDOWS, IS_LINUX } = require('./constants')
const preload = BOOT
  ...
  async popup ({ x = 0, y = 0 } = {}) {
    ...
    if (this.dev) {
      items.push(new electron.MenuItem({
        ...
        click: () => this.webContents.inspectElement(x, y)
      }))
    }
...
class App {
  ...
  async start (ipc) {
    ...
    try {
      ...
      const { dev, trace, key, tools, stage } = ctx
      ...
      await gui.ctrl('window', entry, { ctx }, {
        ...
        afterNativeViewCreated: tools && ((app) => {
...
const joyredir = NETWORK_RECORD ? (url, ua) => `about:blank#{"url": "${url}", "ua": "${ua}"}` : null
...
class GuiCtrl {
  ...
  constructor (entry, options, { ctx, parentId, ua, sessname, appkin }) {
    ...
    this.appkin = appkin
    if (ALIASES.keet.z32 === this.ctx.key?.z32) {
      if (this.ctx.options.platform && typeof this.ctx.options.platform === 'object') {
        this.ctx.options.platform = {
          ...this.ctx.options.platform,
          __legacyTitlebar: typeof this.ctx.options.platform.__legacyTitlebar === 'boolean' ? this.ctx.options.platform.__legacyTitlebar : true
        }
      } else {
        this.ctx.options.platform = { __legacyTitlebar: true }
      }
    }
    this.tbh = this.ctx.options.platform?.__legacyTitlebar ? 48 : 0
  }
...
class Window extends GuiCtrl {
  ...
  async open (opts = {}) {
    ...
    this.win = new BrowserWindow({
      ...
      ...(IS_MAC && this.ctx.options.platform?.__legacyTitlebar ? { titleBarStyle: 'hidden', trafficLightPosition: { x: 12, y: 16 }, titleBarOverlay: true } : {}),
    ...
    if (IS_MAC && !this.ctx.options.platform?.__legacyTitlebar) {
      this.win.on('enter-full-screen', () => { this.win.setWindowButtonVisibility(true) })
      this.win.on('will-resize', () => { // leave-full-screen is too late, will-resize with fullscreen check is in time
        if (this.win.isFullScreen()) return
        this.win.setWindowButtonVisibility(false)
        this.win.webContents.executeJavaScript('window.dispatchEvent(new Event(\'__macos-exit-fullscreen\'))')
        this.view?.webContents.executeJavaScript('window.dispatchEvent(new Event(\'__macos-exit-fullscreen\'))')
      })
    }
    ...
    if (decal === false) {
      ...
      await this.win.loadURL(joyredir ? joyredir(this.entry, ua) : this.entry)
      this.opening = false
      this.closed = false
      return !this.closed
    }
    ...
    const decalLoading = this.win.loadURL(joyredir ? joyredir(decalUrl, 'Pear Platform') : decalUrl)
    ...
    const viewLoading = this.view.webContents.loadURL(joyredir ? joyredir(this.entry, ua) : this.entry)
    viewInitialized()
...
class View extends GuiCtrl {
  ...
  async open (opts = {}) {
    ...
    await this.view.webContents.loadURL(joyredir ? joyredir(this.entry, ua) : this.entry)
