...
const { FORCE_SHOW, ALIASES, IS_MAC, IS_WINDOWS } = require('./constants')
...
class App {
  ...
  async start (ipc) {
    ...
    try {
      ...
      if (key === null && !ctx.updates && ctx.options.watch && !ctx.options.watch.disable) {
        changes = require('./watch.js')
      }
