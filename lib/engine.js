...
class Engine extends ReadyResource {
  ...
  async * stage ({ channel, key, dir, dryRun, name, data = false, clientArgv, ignore = '.git,.github,.DS_Store' }, client) {
    const session = new Session(client)
    ...
    try {
      ...
      await ctx.initialize({ bundle, dryRun, data })
      const type = data ? 'data' : ctx.manifest.pear?.type || 'desktop'
      if (data) ignore = []
      else if (ctx.manifest.pear?.stage?.ignore) ignore = ctx.manifest.pear.stage?.ignore
      ...
      yield { tag: 'staging', data: { data, name: ctx.name, channel: bundle.channel, key: pearkey, current: currentVersion, release } }

      if (dryRun) yield { tag: 'dry' }

      const root = unixPathResolve(ctx.cwd)
      const main = unixPathResolve('/', ctx.main)
      const src = new LocalDrive(root, { followLinks: true, metadata: new Map() })
      const dst = bundle.drive
      const opts = { filter: (key) => ignore.some((path) => key.startsWith(path)) === false, dryRun, batch: true }

      const builtins = type === 'terminal' ? gunk.bareBuiltins : gunk.builtins
      ...
      yield { tag: 'addendum', data: { version: bundle.version, release, channel, key: pearkey, type } }
