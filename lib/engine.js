...
const { CHECKOUT, BOOT, SWAP, PLATFORM_DIR, DHT_BOOTSTRAP, RUNTIME, DESKTOP_RUNTIME, IS_BARE } = require('./constants')

const SWARM_DELAY = 5000
const APPLINGS_PATH = path.join(PLATFORM_DIR, 'applings')

const contentType = (filepath) => {
  const ct = mime.contentType(path.extname(filepath) || 'application/javascript; charset=utf-8')
  if (ct === false) return 'application/octet-stream'
  return ct.replace('application/node', 'application/javascript')
}
...
class Engine extends ReadyResource {
  ...
  constructor (sidecar, { updater, drive, corestore }) {
    ...
    this.drive = drive
    this.replicator = this.drive.core ? new Replicator(this.drive, { appling: true }) : null
    ...
    this.App = class App {
      ...
      async compile (entry = this.ctx.main) {
        ...
        for await (const data of new DependencyStream(this.bundle.drive, { packages: true, source: true, entrypoint: entry })) {
          if (first) {
            first = false
            entry = data.key
          }
          ...
          if (data.resolutions.length) {
            const res = resolutions[data.key] = {}
            for (const r of data.resolutions) {
              if (!r.input || !r.output) continue
              res[r.input] = r.output
            }
          }

          for (const addon of data.addons) {
            pendingAddons.push(this._loadUnsafeAddon(this.bundle.drive, addon.input, addon.output))
          }
        }

        for (const addon of await Promise.all(pendingAddons)) {
          addons[addon.input] = addon.output
        }
  ...
  async lookup (app, protocol, type, req, res) {
    ...
    let isJS = false
    if (protocol !== 'resolve') {
      const ct = contentType(link.filename)
  ...
  async #ensureSwarm () {
    ...
    this.swarm = new Hyperswarm({ bootstrap: DHT_BOOTSTRAP, keyPair: this.keyPair })
  ...
  async #start (session, client, argv, env, cwd, startId) {
    ...
    if (ctx.key === null && ctx.dev === false) {
      const err = new Error(`Invalid Key: ${ctx.flags.run}`)
    ...
    if (ctx.key === null) {
      ...
      const bundle = new Bundle({ drive })
      const linker = new AppLinker(bundle)
      ...
      app.bundle = bundle
      try {
        await ctx.initialize({ bundle, app, staging: true })
      } catch (err) {
        if (err.code === 'ERR_CONNECTION') app.report({ err })
      }
      const updating = await app.minver()
      const type = ctx.options.type
      const src = type === 'terminal' ? await app.compile() : null
      return { port: this.port, id, startId, host: `http://127.0.0.1:${this.port}`, bail: updating, type, src }
    }
    ...
    const initializing = ctx.initialize({ bundle, app })

    if (bundle.platformVersion !== null) {
      app.report({ type: 'upgrade' })
      const type = ctx.options.type
      const src = type === 'terminal' ? await app.compile() : null
      return { port: this.port, id, startId, host: `http://127.0.0.1:${this.port}`, type, src }
    }
