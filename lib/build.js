...
class RewritingLocalDrive extends LocalDrive {
  constructor (root, rewrite, opts = {}) {
    super(root, opts)
    this.rewrite = rewrite ?? (key => key)
  }

  put (key, value, opts = {}) { return super.put(this.rewrite(key), value, opts) }
  del (key, opts = {}) { return super.del(this.rewrite(key), opts) }
  async symlink (key, linkname) { return super.symlink(this.rewrite(key), this.rewrite(linkname)) }
  async entry (name, opts) { return super.entry(this.rewrite(name), opts) }
  createWriteStream (key, opts) { return super.createWriteStream(this.rewrite(key), opts) }
}
...
async function * installCmakeFiles ({ dir, drive }) {
  yield { tag: 'installingCmakeFiles' }

  const prefix = '/node_modules/bare-dev/cmake'
  const rewrite = (key) => key.replace(prefix, '/cmake') || '/'

  const dest = new RewritingLocalDrive(dir, rewrite)
  const mirror = new MirrorDrive(drive, dest, { prefix, dryRun: false, prune: true })

  for await (const diff of mirror) {
    const message = rewrite(diff.key)

    if (diff.op === 'add') {
      yield { tag: 'byte-diff', data: { type: 1, sizes: [diff.bytesAdded], message } }
    } else if (diff.op === 'change') {
      yield { tag: 'byte-diff', data: { type: 0, sizes: [-diff.bytesRemoved, diff.bytesAdded], message } }
    } else if (diff.op === 'remove') {
      yield { tag: 'byte-diff', data: { type: -1, sizes: [-diff.bytesRemoved], message } }
    }
  }
}
