...
module.exports = async function * ({ key, dir }) {
  yield { tag: 'initializing' }
  if (checkFile(path.resolve(dir, 'CMakeLists.txt'))) {
    yield { tag: 'cmakeExists' }
  } else {
    await init.appling({ cwd: dir, key, quiet: false, ...loadApplingOpts(dir) })
  ...
  const configureProcess = configure({ source: dir, cwd: dir, verbose: false, quiet: false, stdio, detached: true })
  for await (const data of handleOutput(configureProcess)) yield data

  yield { tag: 'building' }
  const buildProcess = build({ cwd: dir, verbose: false, quiet: false, stdio, detached: true })
...
async function * handleOutput (child) {
  ...
  for await (const { tag, data } of mergedStream) {
    if (tag === 'exit') {
      if (data !== 0) throw new Error(`Build failed with exit code ${data.data}`)
      break
    }
