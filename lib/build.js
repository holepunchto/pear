...
async function * build ({ key, dir, pkg }) {
  ...
  const define = ['BARE_SCRIPT_INTERPRETER=pear.dev', 'BARE_SCRIPT_INTERPRETER_ARGS=run pear://88kfah6czh8tpyo68sph8sh5jfitcmmo4bd3x1utmtgqi6p1c3qo']
  const configureProcess = configureApp({ source: dir, cwd: dir, verbose: false, quiet: false, stdio, detached: true, define })
  for await (const data of handleOutput(configureProcess)) yield data

  yield { tag: 'building' }
  const buildProcess = buildApp({ cwd: dir, verbose: false, quiet: false, stdio, detached: true, define })
