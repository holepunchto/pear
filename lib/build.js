...
async function * installBareDevFiles ({ dir, drive }) {
  yield { tag: 'installingBareDevFiles' }

  const prefix = '/node_modules/bare-dev'
  const dest = new LocalDrive(unixPathResolve(dir))

  const bareDev = await dest.entry(unixPathResolve(prefix, 'package.json'))
  if (bareDev?.key) {
    yield { tag: 'bareDevExists' }
    return
  }
  ...
  await dest.symlink('/cmake', unixPathResolve(prefix, 'cmake'))
}
