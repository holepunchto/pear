...
async function * installBareDev ({ dir }) {
  yield { tag: 'installingBareDev' }

  const hasGlobalBareDev = await which('bare-dev', { nothrow: true })
  const hasCmakeDir = checkFile(path.resolve(dir, 'cmake'))

  if (hasGlobalBareDev && hasCmakeDir) {
    yield { tag: 'bareDevExistsInPath' }
    return
  }

  const npmPath = await which('npm')
  if (hasGlobalBareDev && npmPath) {
    const npmRootPath = await getNpmRootPath({ dir, npmPath })
    if (npmRootPath && checkFile(path.resolve(npmRootPath, 'bare-dev', 'package.json'))) {
      await fs.promises.symlink(path.resolve(npmRootPath, 'bare-dev', 'cmake'), path.resolve(dir, 'cmake'), 'dir')
      yield { tag: 'bareDevExistsInPath' }
      return
    }
  }

  if (!npmPath) throw Object.assign(new Error('npm not found in PATH'), { code: 'ENOENT' })

  if (!checkFile(path.resolve(dir, 'node_modules', 'bare-dev', 'package.json'))) {
    const npmInstall = spawn(npmPath, ['install', 'bare-dev'], { cwd: dir, stdio: ['ignore', 'pipe', 'pipe'] })
    for await (const data of handleOutput(npmInstall)) yield data

    yield { tag: 'bareDevInstalled' }
  } else {
    yield { tag: 'bareDevExistsInApp' }
  }

  if (!hasCmakeDir) {
    await fs.promises.symlink(path.resolve(dir, 'node_modules', 'bare-dev', 'cmake'), path.resolve(dir, 'cmake'), 'dir')
  }
}

async function getNpmRootPath ({ npmPath }) {
  const npmRoot = spawn(npmPath, ['root', '-g'], { stdio: ['ignore', 'pipe', 'pipe'] })
  let npmRootPath = ''
  for await (const { tag, data } of handleOutput(npmRoot)) if (tag === 'stdout') npmRootPath += data.trim()
...
module.exports = {
  build,
  checkApplingOpts,
  installBareDevFiles: installBareDev
}
