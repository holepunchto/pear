...
class Engine extends ReadyResource {
  ...
  async * info ({ key, channel, dir, showKey, metadata, changelog, full } = {}, client) {
    const session = new Session(client)
    let bundle = null
    const anyFlag = [changelog, full, metadata, showKey].some(flag => flag === true)
    const display = (flag) => anyFlag ? !!flag : flag !== false
    try {
      if (key) {
        ...
        await bundle.ready()
        if (display(showKey)) yield { tag: 'retrieving', data: { hex, z32 } }
      } else if (channel) {
      ...
      const parsed = showChangelog === 'latest'
        ...
        : showChangelog === 'full'
          ? (await clog.parse(contents).map(entry => entry[1]).join('\n\n')) || blank
          : blank

      if (showChangelog) yield { tag: 'changelog', data: { changelog: parsed, full } }
    } catch (err) {
      yield { tag: 'error', data: { ...err, stack: err.stack, message: err.message, code: err.code } }
    } finally {
      yield null
  ...
  async * gc ({ pid, cmd }, client) {
    const session = new Session(client)
    try {
      if (cmd === 'sidecar') {
        const pids = [Bare.pid, pid]
        const name = 'pear-runtime'
        const flag = '--sidecar'
        const killed = []
        yield { tag: 'retrieving', data: { name, flag } }
        const stream = await this.#gc.sidecar({ name, flag })
        let buffer = ''
        for await (const chunk of stream) {
          const { tag, data } = chunk
          if (typeof chunk === 'string') {
            buffer += chunk
            const lines = buffer.split(isWindows ? '\r\r\n' : '\n')
            buffer = lines.pop()
            for (const line of lines) {
              const columns = line.split(isWindows ? ',' : ' ').filter(col => col)
              const pidIndex = isWindows ? 4 : 0
              const flagIndex = columns.findIndex(col => col.includes(flag))
              if (flagIndex !== -1 && columns.length > pidIndex) {
                const id = parseInt(columns[pidIndex])
                if (!isNaN(id) && !pids.includes(id)) {
                  const ok = await this.#gc.kill(id)
                  if (ok) {
                    yield { tag: 'kill', data: { pid: id } }
                    killed.push(id)
                  }
                }
              }
            ...
            yield { tag: 'error', data: { message: chunk.data } }
          }
        }
        yield { tag: 'complete', data: { killed } }
      } else {
  ...
  async #moveStorage (params) {
    if (!params.src) throw ERR_SHIFT_STORAGE_ERROR('src must be specified')
    if (!params.dst) throw ERR_SHIFT_STORAGE_ERROR('dst must be specified')
    const src = parse.runkey(params.src)
    const dst = parse.runkey(params.dst)
    if (src.key === null) throw ERR_SHIFT_STORAGE_ERROR('Invalid source app key')
    if (dst.key === null) throw ERR_SHIFT_STORAGE_ERROR('Invalid destination app key')
    ...
    try {
      if (await exists(from) === false) {
        throw ERR_SHIFT_STORAGE_ERROR('No app storage for found for ' + params.src)
      }
      if (await exists(to)) {
        if (params.force) {
          ...
          throw ERR_SHIFT_STORAGE_ERROR('App storage for ' + params.dst + ' already exists. Use --force to overwrite')
        }
      }
...
class GarbageCollector {
  async sidecar ({ name = 'pear-runtime', flag = '--sidecar' } = {}) {
    if (!name || !flag) throw new Error('Process name or flag is missing')
    const stream = new Readable({ objectMode: true })
    const cmd = isWindows
      ? `wmic process where (name like '%${name}%') get name,executablepath,processid,commandline /format:csv`
      : `ps ax | grep -i -- '${name}' | grep -i -- '${flag}'`
    ...
    stream.push({ tag: 'pid', data: { pid: sp.pid } })
    sp.stdout.on('data', (data) => { stream.push(data.toString()) })
    sp.stderr.on('data', (data) => { stream.push({ tag: 'error', data: data.toString() }) })

    sp.on('exit', (code, signal) => {
      if (code !== 0 || (code === null && signal)) {
        stream.push({ tag: 'error', data: `Process exited with code: ${code}` })
      }
      stream.push(null)
    })
    return stream
  ...
  async kill (pid) {
    if (!pid) return Promise.reject(new Error('Invalid pid'))

    const command = isWindows ? 'taskkill' : 'kill'
    const args = isWindows ? ['/F', '/PID', pid.toString()] : [pid.toString()]

    return new Promise((resolve, reject) => {
      const sp = spawn(command, args)
      sp.on('error', (err) => { reject(new Error(`Error when trying to kill process ${pid}:`, err)) })
      sp.on('exit', (code, signal) => {
        if (!signal && (code === 0 || code == null)) {
          resolve(pid)
        } else {
          reject(new Error(`Failed to kill process ${pid}. Exit code: ${code}, Signal: ${signal || 'none'}`))
        }
      })
    })
  }
...
// class GarbageCollector {
//   async sidecar ({ name = null, flag = null }) {
//     if (!name || !flag) throw new Error('Process name or flag is missing')
//
//     const stream = new Readable({ objectMode: true })
//
//     const [sh, args] = isWindows
//       ? ['cmd.exe', ['/c', `wmic process where "executablepath like '%${name}%'" get processid, commandline`]]
//       : ['/bin/sh', ['-c', `ps ax | grep -i -- '${name}' | grep -i -- '${flag}'`]]
//
//     const sp = spawn(sh, args)
//     sp.stderr.on('data', (data) => { stream.push({ tag: 'error', data }) })
//     sp.stdout.on('data', (data) => {
//       const lines = data.toString().trim().split('\n')
//       lines.forEach(line => {
//         if (line.toLowerCase().includes(flag.toLowerCase())) {
//           const match = line.match(/\d+/)
//           if (match) {
//             const pid = Number(match[0])
//             if (pid !== sp.pid) stream.push({ tag: 'pid', data: { pid } })
//           }
//         }
//       })
//     })
//
//     sp.on('error', (data) => { stream.push({ tag: 'error', data }) })
//     sp.on('exit', (code, signal) => {
//       if (code !== 0 || (code === null && signal)) {
//         stream.push({ tag: 'error', data: `Process exited with code: ${code}, Signal: ${signal || 'none'}` })
//       }
//       stream.push(null)
//     })
//
