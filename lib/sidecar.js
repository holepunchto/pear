...
const { pipelinePromise: pipeline, Readable } = require('streamx')
...
class Engine extends ReadyResource {
  ...
  build ({ key, dir }, client) {
    const stream = new Readable()

    const doBuild = async () => {
      await this.ready()
      stream.push({ tag: 'starting', data: { platform: os.platform(), arch: os.arch(), key, dir } })

      for await (const data of this.dump({ id: Bare.pid, key, dir, finalize: false }, client)) stream.push(data)

      const build = await subsystem(this.drive, '/subsystems/build.js')
      await build({ key, dir, stream })
    }

    doBuild()
      .then(() => stream.push({ tag: 'final', data: { success: true } }))
      .catch(({ stack, code, message }) => {
        stream.push({ tag: 'error', data: { stack, code, message, success: false } })
        stream.push({ tag: 'final', data: { success: false } })
      })
      .finally(async () => stream.push(null))
