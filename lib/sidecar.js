...
class Engine extends ReadyResource {
  ...
  shift (params, client) { return new Shift(params, client, this) }
  ...
  async #start (flags, client, session, env, link, dir, startId, args) {
    ...
    const appBundle = new Bundle({
      corestore: this.engine._getCorestore(ctx.manifest?.name, ctx.channel),
      appling: ctx.appling,
      channel: ctx.channel,
...
class OpStream extends streamx.Readable {
  constructor (op, params, client, engine) {
    super({
      read (cb) {
        const error = (err) => {
...
class Release extends OpStream {
  ...
  async #op ({ name, channel, checkout, link, dir }) {
    ...
    if (await bundle.db.get('manifest') === null) {
      this.push({ tag: 'final', data: { reason: `The "${name}" app has not been staged on ${channel ? '"' + channel + '" channel' : link}.`, success: false } })
      return
    }
...
class Stage extends OpStream {
  ...
  async #op ({ channel, key, dir, dryRun, name, truncate, bare = false, clientArgv, ignore = '.git,.github,.DS_Store' }) {
    ...
    const root = unixPathResolve(ctx.dir)
...
class Info extends OpStream {
  ...
  async #op ({ link, channel, dir, showKey, metadata, changelog, full } = {}) {
    const { session } = this
    let bundle = null
    ...
    if (link) {
      ...
      const corestore = this.engine._getCorestore(null, null)
      bundle = new Bundle({ corestore, key })
      await bundle.ready()
      if (isEnabled(showKey)) this.push({ tag: 'retrieving', data: { hex, z32 } })
    } else if (channel) {
      const ctx = new Context({ flags: { channel, link }, dir })
      const corestore = this.engine._getCorestore(ctx.name, channel)
      bundle = new Bundle({ corestore, channel })
      await bundle.ready()
      const hex = bundle.drive.key.toString('hex')
      const z32 = hypercoreid.encode(bundle.drive.key)
      if (isEnabled(showKey)) this.push({ tag: 'retrieving', data: { hex, z32 } })
    } else if (this.engine.drive.key) {
      const hex = this.engine.drive.key.toString('hex')
      const z32 = hypercoreid.encode(this.engine.drive.key)
      if (isEnabled(showKey)) this.push({ tag: 'retrieving', data: { hex, z32 } })
    }
    await this.engine.ready()
    if (bundle) {
      await session.add(bundle)
      await bundle.join(this.engine.swarm)
    }
    const drive = bundle?.drive || this.engine.drive
