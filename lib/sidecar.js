...
class Engine extends ReadyResource {
  ...
  async * info ({ key, channel, dir, showKey, keys, metadata, changelog, full } = {}, client) {
    console.log('sidecar | metadata:', metadata)
    const session = new Session(client)
    const anyFlag = [changelog, full, metadata, showKey, keys].some(flag => flag === true)
    ...
    try {
      ...
      switch (showChangelog) {
        case 'latest': {
          parsed = await clog.parse(contents).at(0)?.[1] || '[ No Changelog ]'
          break
        }
        case 'full': {
          const entries = await clog.parse(contents)
          parsed = entries.map(entry => entry[1]).join('\n') || '[ No Changelog ]'
          break
        }
        default:
          parsed = '[ No Changelog ]'
