...
class Sidecar {
  ...
  constructor ({ updater, drive, corestore }) {
    this.verbose = global.Bare.argv.includes('--verbose')
  ...
  async restart ({ platform = false } = {}, client) {
    ...
    if (platform === false) {
      const { dir, argv, env } = client.userData.ctx
  ...
  closeClients () {
    ...
    for (const client of this.clients) {
      ...
      const { pid, clientArgv, dir, runtime, appling, argv, env } = app.ctx
...
class Engine extends ReadyResource {
  ...
  constructor (sidecar, { updater, drive, corestore }) {
    ...
    this.App = class App {
      ...
      messages (ptn) {
        const subscriber = this.engine.bus.sub({ topic: 'messages', id: this.id, ...(ptn ? { data: ptn } : {}) })
  ...
  seed (params, client) { return new Seed(params, client, this) }

  release (params, client) { return new Release(params, client, this) }

  stage (params, client) { return new Stage(params, client, this) }

  dump (params, client) { return new Dump(params, client, this) }

  info (params, client) { return new Info(params, client, this) }
  ...
  gc ({ pid, resource }, client) {
    ...
    if (resource === 'release') gc.release({ resource })
    else if (resource === 'sidecar') gc.sidecar({ pid, resource })
  ...
  async * shift (params, client) {
    ...
    try {
      yield { tag: 'moving', data: { src: params.src, dst: params.dst } }
      const { from, to } = await this.#moveStorage(params)
      yield { tag: 'complete', data: { from, to, src: params.src, dst: params.dst } }
    } catch (err) {
  ...
  async #moveStorage (params) {
    if (!params.src) throw ERR_INVALID_INPUT('src must be specified')
    if (!params.dst) throw ERR_INVALID_INPUT('dst must be specified')
    const src = parseLink(params.src)
    const dst = parseLink(params.dst)
    if (src.key === null) throw ERR_INVALID_INPUT('Invalid source app key')
    ...
    const from = path.join(byDkey, discoveryKey(src.key.buffer).toString('hex'))
    const to = path.join(byDkey, discoveryKey(dst.key.buffer).toString('hex'))
    const exists = (path) => fs.promises.stat(path).then(() => true, () => false)
    let gc = null
    try {
      if (await exists(from) === false) {
        throw ERR_INVALID_INPUT('No app storage for found for ' + params.src)
      }
      if (await exists(to)) {
        if (params.force) {
          ...
          throw ERR_INVALID_INPUT('App storage for ' + params.dst + ' already exists. Use --force to overwrite')
        }
      }
  ...
  async #start (flags, client, session, env, link, dir, startId, args) {
    ...
    const ctx = new Context({ id, env, link, dir, flags, args })
    ...
    if (ctx.key === null) {
      const drive = new LocalDrive(unixPathResolve(ctx.dir), { followLinks: true })
    ...
    const appBundle = new Bundle({
      corestore: this._getCorestore(ctx.manifest?.name, ctx.channel),
      appling: ctx.appling,
      channel: ctx.channel,
      checkout: ctx.checkout,
      key: ctx.key?.hex,
      name: ctx.manifest?.name,
      dir: ctx.key ? null : ctx.dir,
      updatesDiff: ctx.updatesDiff,
      trace,
      updateNotify: ctx.updates && ((version, info) => this.sidecar.updateNotify(version, info)),
...
class DataStream extends streamx.Transform {
  constructor () {
    super({
      objectMode: true,
      transform ({ data }, cb) {
...
class OpStream extends streamx.Readable {
  constructor (init, params, client, engine) {
    super({
      read (cb) {
        const error = (err) => {
        ...
        init(params).catch(error).finally(close)
      }
    })
    this.client = client
    this.engine = engine
...
class Release extends OpStream {
  constructor (...args) {
    super((...args) => this.#init(...args), ...args)
  }

  async #init ({ name, channel, checkout, key, dir }) {
    ...
    this.push({ tag: 'releasing', data: { name, channel: channelName } })

    const corestore = this.engine._getCorestore(name || ctx.name, channel, { writable: true })

    const bundle = new Bundle({ corestore, channel, key })
    await session.add(bundle)

    if (await bundle.db.get('manifest') === null) {
      this.push({ tag: 'final', data: { reason: `The "${name}" app has not been staged on "${channel}" channel.`, success: false } })
      return
    }
    ...
    this.push({ tag: 'released', data: { name, channel, length: bundle.db.feed.length } })
  }
}

class Stage extends OpStream {
  ...
  constructor (...args) { super((...args) => this.#init(...args), ...args) }

  async #init ({ channel, key, dir, dryRun, name, truncate, bare = false, clientArgv, ignore = '.git,.github,.DS_Store' }) {
    const { client, session, engine } = this
    const ctx = new Context({
    ...
    const corestore = engine._getCorestore(name || ctx.name, channel, { writable: true })
    ...
    this.push({ tag: 'staging', data: { name: ctx.name, channel: bundle.channel, key: pearkey, current: currentVersion, release } })
...
class Seed extends OpStream {
  constructor (...args) { super((...args) => this.#init(...args), ...args) }

  async #init ({ name, channel, link, verbose, seeders, dir, clientArgv } = {}) {
    const { client } = this
    const session = new Session(client)
    try {
      const ctx = new Context({
      ...
      client.userData = new this.engine.App({ ctx, session })

      this.push({ tag: 'seeding', data: { key: link, name, channel } })
      await this.engine.ready()

      const corestore = this.engine._getCorestore(name, channel)
      ...
      const log = (msg) => this.engine.bus.pub({ topic: 'seed', id: client.id, msg })
      const notices = this.engine.bus.sub({ topic: 'seed', id: client.id })
      ...
      await bundle.join(this.engine.swarm, { seeders, server: true })

      for await (const { msg } of notices) this.push(msg)
    } catch (err) {
      this.push({ tag: 'error', data: { ...err, stack: err.stack, message: err.message, code: err.code } })
      this.push(null)
      await session.close()
    }
...
class Dump extends OpStream {
  constructor (...args) { super((...args) => this.#init(...args), ...args) }

  async #init ({ key, dir, checkout }) {
    const { session, engine } = this
    await engine.ready()
    if (key) key = hypercoreid.decode(key)
    checkout = Number(checkout)
    const corestore = engine._getCorestore(null, null)
    const bundle = new Bundle({ corestore, key, checkout })

    await session.add(bundle)

    if (engine.swarm) bundle.join(engine.swarm)
...
class Info extends OpStream {
  ...
  async #init ({ key } = {}) {
    const { session, engine } = this
    if (key) {
      const parsed = parseLink(key)
      key = parsed.key.buffer
      const hex = parsed.key.hex
      const z32 = parsed.key.z32
      this.push({ tag: 'retrieving', data: { hex, z32 } })
    } else if (engine.drive.key) {
      const hex = engine.drive.key.toString('hex')
      const z32 = hypercoreid.encode(engine.drive.key)
      this.push({ tag: 'retrieving', data: { hex, z32 } })
    }
    await engine.ready()
    const corestore = engine._getCorestore(null, null)
    const bundle = key ? new Bundle({ corestore, key }) : null
    if (bundle) await session.add(bundle)
    const drive = bundle?.drive || engine.drive
    if (bundle) await bundle.ready()

    if (drive.key && drive.contentKey && drive.discoveryKey) {
      this.push({ tag: 'keys', data: { project: drive.key.toString('hex'), content: drive.contentKey.toString('hex'), discovery: drive.discoveryKey.toString('hex') } })
      ...
      this.push({ tag: 'info', data: { channel, release, name, live: bundle?.live || false } })
    }
    ...
    this.push({ tag: 'changelog', data: { changelog } })
  }
}
