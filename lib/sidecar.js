...
class Engine extends ReadyResource {
  ...
  async * dump ({ key, dir, checkout, finalize = true }, client) {
    const session = new Session(client)
    try {
      await this.ready()
      if (key) key = hypercoreid.decode(key)
  ...
  async * build ({ key, dir }, client) {
    const session = new Session(client)

    try {
      await this.ready()
      yield { tag: 'starting', data: { platform: os.platform(), arch: os.arch(), key, dir } }

      const { build, installBareDevFiles, checkApplingOpts } = await subsystem(this.drive, '/subsystems/build.js')

      const bundle = new Bundle({ corestore: this.#getCorestore(), key: hypercoreid.decode(key) })
      await session.add(bundle)
      if (this.swarm) bundle.join(this.swarm)
      await bundle.calibrate()

      const rawPkg = await bundle.drive.get('/package.json')
      if (!rawPkg) throw Object.assign(new Error('No package.json found in the app'), { code: 'ERR_NOENT' })

      let pkg
      try {
        pkg = JSON.parse(rawPkg?.toString())
      } catch (err) {
        throw Object.assign(new Error('Failed to parse package.json'), { code: 'ERR_INVALID_JSON' })
      }

      await checkApplingOpts(pkg)

      for await (const data of this.dump({ id: Bare.pid, key, dir, finalize: false }, client)) yield data

      for await (const data of installBareDevFiles({ dir, drive: this.drive })) yield data

      for await (const data of build({ key, dir, pkg })) yield data

      yield { tag: 'final', data: { success: true } }
    } catch ({ stack, code, message }) {
      yield { tag: 'error', data: { stack, code, message, success: false } }
      yield { tag: 'final', data: { success: false } }
    }
  ...
  async #moveStorage (params) {
    if (!params.src) throw new Error('src must be specified')
    if (!params.dst) throw new Error('dst must be specified')
    const src = parse.runkey(params.src)
    const dst = parse.runkey(params.dst)
    if (src.key === null) throw new Error('Invalid source app key')
    if (dst.key === null) throw new Error('Invalid destination app key')
    ...
    try {
      if (await exists(from) === false) {
        throw Object.assign(
          new Error('No app storage for found for ' + params.src),
          { code: 'ERR_NOENT' }
        )
      }
      if (await exists(to)) {
        if (params.force) {
          ...
          throw Object.assign(
            new Error('App storage for ' + params.dst + ' already exists. Use --force to overwrite'),
            { code: 'ERR_EXISTS' }
          )
        }
      }
