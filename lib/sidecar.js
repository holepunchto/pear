...
class Sidecar {
  ...
  constructor ({ updater, drive, corestore }) {
    this.log = new Logger(global.Bare.argv)
    ...
    this.ipc = new IPC({
      ...
      methods: require('../gui/methods'),
      socketPath: SOCKET_PATH
    })

    this.ipc.on('client', (client) => {
      client.id = this.freelist.alloc(client)
      client.once('close', () => {
    ...
    this.closing.finally(() => this.log.info((isWindows ? '^' : '✔') + ' Sidecar closed'))
    this.#closed = closed
    this.#spindownCountdown()
  }
  ...
  #spindownCountdown () {
    ...
    this.spindownt = setTimeout(async () => {
      if (this.freelist.emptied() === false) return
      this.close().catch(this.log.error)
    }, this.spindownms)
  }

  get clients () { return this.freelist.alloced.map(Boolean) }
  ...
  async updateNotify (version, info = {}) {
    ...
    if (info.link) {
      this.log.info('Application update available:')
    } else if (version.force) {
      this.log.info('Platform Force update (' + version.force.reason + '). Updating to:')
    } else {
      this.log.info('Platform update Available. Restart to update to:')
    }

    this.log.info('  v' + version.fork + '.' + version.length + '.' + version.key)
  ...
  async restart ({ platform = false } = {}, client) {
    this.log.info('Restarting ' + (platform ? 'platform' : 'client'))
    if (platform === false) {
    ...
    this.log.info('Restarting', restarts.length, 'apps')
  ...
  async shutdown (client) {
    this.log.info('- Sidecar shutting down...')
    const app = client.userData
    const tearingDown = !!app && app.teardown()
    if (tearingDown === false) client.close()
  ...
  async close () {
    ...
    this.closeClients()
    await engineClosing
    for (const ipc of this.clients) await ipc.close
    await this.ipc.close()
    if (this.updater) {
      if (await this.updater.applyUpdate() !== null) {
        this.log.info((isWindows ? '^' : '✔') + ' Applied update')
      }
    }
    this.#closed()
  }
  ...
  deathClock (ms = 20000) {
    ...
    this.bailout = setTimeout(() => {
      this.log.error('DEATH CLOCK TRIGGERED, FORCE KILLING. EXIT CODE 124')
...
class Engine extends ReadyResource {
  ...
  constructor (sidecar, { updater, drive, corestore }) {
    ...
    this.App = class App {
      ...
      #mapReport (report) {
        ...
        if (report.err) this.log.error('REPORT', report.err) // send generic errors to the text error log as well
    ...
    this.lazySwarmTimeout = setTimeout(() => {
      ...
      this.ready().catch((err) => this.log.error(err))
    }, SWARM_DELAY)
  }
    ...
    this.server = http.createServer(async (req, res) => {
      try {
        ...
        if (err.code === 'MODULE_NOT_FOUND') {
          ...
          this.log.error('Unknown Server Error', err)
          err.status = 500
        }
  ...
  async * stage ({ channel, key, dir, dryRun, name, truncate, bare = false, clientArgv, ignore = '.git,.github,.DS_Store' }, client) {
    ...
    try {
      ...
      const bundle = new Bundle({
        ...
        truncate,
        stage: true,
        failure (err) { this.log.error(err) }
      })
      await session.add(bundle)
  ...
  wakeup (...args) { return this.sidecar.wakeup(...args) }
  ...
  async setPreference ({ key, version }) {
