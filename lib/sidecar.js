...
class Engine extends ReadyResource {
  ...
  async * seed ({ name, channel, link, verbose, seeders, dir, clientArgv } = {}, client) {
    ...
    try {
      ...
      yield { tag: 'seeding', data: { key: link, name, channel } }
      await this.ready()
  ...
  async * #trace (bundle, client) {
    ...
    console.log(DESKTOP_RUNTIME, [BOOT, `--trace=${client.id}`, '--run', bundle.drive.key.toString('hex'), '--swap', SWAP])
    const sp = spawn(DESKTOP_RUNTIME, [BOOT, `--trace=${client.id}`, '--run', bundle.drive.key.toString('hex'), '--swap', SWAP], {
      stdio: 'inherit',
      env: {
        ...ENV,
  ...
  async * info ({ key, channel, dir, showKey, metadata, changelog, full } = {}, client) {
    ...
    try {
      if (key) {
        ...
        const ctx = new Context({ argv: [`--channel=${channel}`], dir })
  ...
  gc ({ pid, resource }, client) {
    if (resource !== 'sidecar') throw ERR_INVALID_GC_RESOURCE('Invalid resource to gc: ' + resource)
  ...
  async #start (flags, client, session, env, link, dir, startId, args) {
    ...
    if (trusted.has(ctx.key.z32) === false) {
      const err = new ERR_PERMISSION_REQUIRED('Permission required to run key')
      err.key = ctx.key
      app.report({ err })
