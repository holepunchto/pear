...
const {
  ...
  SWAP, RUNTIME, DESKTOP_RUNTIME, SPINDOWN_TIMEOUT
} = require('./constants')
...
class Engine extends ReadyResource {
  ...
  async start (params, client) {
    const { argv, env, cwd, detached } = params
    let { startId } = params
    ...
    const running = this.#start(session, client, argv, env, cwd, startId, detached)
  ...
  async #start (session, client, argv, env, cwd, startId, detached) {
    ...
    if (ctx.key === null) {
      ...
      try {
        ...
        if (err.code === 'ERR_PERMISSION_REQUIRED') app.report({ err })
      }
