...
class Engine extends ReadyResource {
  ...
  async * build ({ key, dir }, client) {
    ...
    try {
      ...
      const { build, installCmakeFiles, checkApplingOpts } = await subsystem(this.drive, '/subsystems/build.js')
      ...
      for await (const data of installCmakeFiles({ dir, drive: this.drive })) yield data
