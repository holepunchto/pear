...
class Engine extends ReadyResource {
  ...
  async * stage ({ channel, key, dir, dryRun, name, truncate, bare = false, clientArgv, ignore = '.git,.github,.DS_Store' }, client) {
    ...
    try {
      ...
      await this.trust({ z32, links: ctx.options?.links || [] })
  ...
  async trust ({ z32, links = null } = {}, client) {
    const trusted = new Set((await preferences.get('trusted')) || [])
    trusted.add(z32)
    if (links && typeof links === 'object') {
      for (const link of Object.values(links)) {
        try {
          trusted.add(hypercoreid.decode(link))
        } catch (err) {
          console.error('Application configuration parsing error while attempting trust', err)
        }
      }
      return await preferences.set('trusted', Array.from(trusted))
    }
    if (!client) return await preferences.set('trusted', Array.from(trusted))
    await this.ready()
    const session = new Session(client)
    const drive = new Hyperdrive(this.#getCorestore(), hypercoreid.decode(z32))
    session.add(drive)
    try {
      await drive.ready()
      let pkg = null
      try {
        ...
        if (err instanceof SyntaxError) throw new Error('Package.json parsing error, invalid JSON')
        console.error('Unexpected Application configuration parsing error while attempting trust', err)
        return await preferences.set('trusted', Array.from(trusted))
      }
      if (pkg && typeof pkg.pear?.links === 'object' && pkg.pear.links) {
  ...
  async #start (session, client, argv, env, cwd, startId) {
    ...
    if (ctx.links) await this.trust({ z32: ctx.key.z32, bundle: appBundle })
