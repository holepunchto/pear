...
class Engine extends ReadyResource {
  ...
  async #moveStorage (params) {
    if (!params.src) throw ERR_SHIFT_STORAGE('src must be specified')
    if (!params.dst) throw ERR_SHIFT_STORAGE('dst must be specified')
    const src = parse.runkey(params.src)
    const dst = parse.runkey(params.dst)
    if (src.key === null) throw ERR_SHIFT_STORAGE('Invalid source app key')
    if (dst.key === null) throw ERR_SHIFT_STORAGE('Invalid destination app key')
    ...
    try {
      if (await exists(from) === false) {
        throw ERR_SHIFT_STORAGE('No app storage for found for ' + params.src)
      }
      if (await exists(to)) {
        if (params.force) {
          ...
          throw ERR_SHIFT_STORAGE('App storage for ' + params.dst + ' already exists. Use --force to overwrite')
        }
      }
