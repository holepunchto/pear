...
const {
  PLATFORM_DIR, GC, SOCKET_PATH, CHECKOUT, APPLINGS_PATH, BOOT,
  SWAP, RUNTIME, DESKTOP_RUNTIME, ALIASES, SPINDOWN_TIMEOUT
} = require('./constants')

const {
  UNRECOGNIZED_START_ID,
  SESSION_IS_CLOSED,
  BAD_REQUEST,
  GONE,
  NOT_FOUND,
...
class Session {
  ...
  async add (resource) {
    ...
    if (this.closed) {
      ...
      throw SESSION_IS_CLOSED('Session is closed')
    }
...
class Engine extends ReadyResource {
  ...
  async lookup (app, protocol, type, req, res) {
    if (app.closed) throw GONE()
    const { bundle, linker } = app
    const url = `${protocol}://${type}${req.url}`
    let link = null
    try { link = ScriptLinker.link.parse(url) } catch { throw BAD_REQUEST(`Bad Request (Malformed URL: ${url})`) }
    ...
    if (await bundle.has(link.filename) === false) {
      ...
      throw NOT_FOUND(`Not Found: "${link.filename}"`)
    }

    if (protocol === 'resolve') {
      res.setHeader('Content-Type', 'text/plain; charset=UTF-8')
      if (!link.resolve && !link.dirname && !link.filename) throw NOT_FOUND(`Not Found: "${req.url}"`)
  ...
  async identify (params, client) {
    if (!client.userData && params.startId) {
      ...
      else throw UNRECOGNIZED_START_ID('identify failure unrecognized startId (check crash logs)')
    }
  ...
  async #serve () {
    this.server = http.createServer(async (req, res) => {
      try {
        ...
        if (ua.slice(0, 4) !== 'Pear') throw BAD_REQUEST()
        ...
        if (protocol !== 'app' && protocol !== 'resolve') {
          throw BAD_REQUEST('Unknown protocol')
        }
        const id = ua.slice(5)
        ...
        if (ipcClient === null) throw BAD_REQUEST('Bad Client ID')

        const app = ipcClient.userData
        if (app.startId !== startId) throw NOT_FOUND()
        if (app.reported?.err) throw NOT_FOUND('Not Found - ' + (app.reported.err.code || 'ERR_UNKNOWN') + ' - ' + app.reported.err.message)
  ...
  async * seed ({ name, channel, key, verbose, seeders, dir, clientArgv } = {}, client) {
    ...
    try {
      ...
      if (await Bundle.provisioned(corestore, key) === false) {
        throw BARE_CORE('Pear Platform: Nothing to seed')
      }
  ...
  async * #trace (bundle, client) {
    ...
    if (status) {
      const err = TRACER_FAILED('Tracer Failed!')
      err.exitCode = status
      throw err
    }
  ...
  async #moveStorage (params) {
    if (!params.src) throw MOVE_STORAGE('src must be specified')
    if (!params.dst) throw MOVE_STORAGE('dst must be specified')
    const src = parse.runkey(params.src)
    const dst = parse.runkey(params.dst)
    if (src.key === null) throw MOVE_STORAGE('Invalid source app key')
    if (dst.key === null) throw MOVE_STORAGE('Invalid destination app key')
    ...
    try {
      if (await exists(from) === false) {
        throw NO_APP_STORAGE('No app storage for found for ' + params.src)
      }
      if (await exists(to)) {
        if (params.force) {
          ...
          throw APP_STORAGE_ALREADY_EXISTS('App storage for ' + params.dst + ' already exists. Use --force to overwrite')
        }
      }
  ...
  async start (params, client) {
    ...
    if (startId && !starting) throw UNRECOGNIZED_START_ID('start failure unrecognized startId')
    const session = new Session(client)
