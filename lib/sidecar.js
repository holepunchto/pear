...
class Engine extends ReadyResource {
  ...
  async * info ({ key, channel, dir, showKey, keys, metadata, changelog } = {}, client) {
    const session = new Session(client)
    try {
      let bundle = null
      if (key) {
        ...
        await bundle.ready()
        if (showKey) yield { tag: 'retrieving', data: { hex, z32 } }
      } else if (channel) {
      ...
      if (drive.key && drive.contentKey && drive.discoveryKey) {
        ...
        if (metadata) yield { tag: 'info', data: { channel, release, name, live: bundle?.live || false } }
      }
      ...
      if (changelog) yield { tag: 'changelog', data: { changelog: parsed, full: changelog === 'full' } }
    } catch (err) {
      yield { tag: 'error', data: { ...err, stack: err.stack, message: err.message, code: err.code } }
    } finally {
      yield null
