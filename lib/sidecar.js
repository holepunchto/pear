...
class Sidecar {
  ...
  async restart ({ platform = false, id = null } = {}, client) {
    ...
    if (platform === false) {
      ...
      if (!client.closed) {
        await new Promise((resolve) => {
          ...
          if (tearingDown === false) client.close(id)
        })
      }
      if (appling) {
...
class Engine extends ReadyResource {
  ...
  async start (params, client) {
    ...
    if (startId && !starting) throw new Error('start failure unrecognized startId: ' + startId)
    const session = new Session(client)
