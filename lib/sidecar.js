...
class Sidecar {
  ...
  constructor ({ updater, drive, corestore }) {
    ...
    this.rpc = new RPC({ handlers: this.engine, socketPath: SOCKET_PATH })

    this.rpc.on('client', (client) => {
      client.id = this.freelist.nextId()
      client.once('close', () => {
  ...
  async close () {
    ...
    this.closeClients()
    await engineClosing
    for (const rpc of this.clients) await rpc.close
...
class Session {
  ...
  async _teardown () {
    ...
    closing.push(this.client.close())
...
class Engine extends ReadyResource {
  ...
  async versions (params, client) {
    return { platform: this.version, app: client.userData?.version }
  }
  ...
  async config (params, client) {
    if (!client.userData) return
    return client.userData.ctx.constructor.configFrom(client.userData.ctx)
  }

  async * reconfig (params, client) {
    if (!client.userData) return
    for await (const { data } of client.userData.reconfiguration()) yield data
  }

  async checkpoint (params, client) {
    if (!client.userData) return
    await fs.promises.writeFile(path.join(client.userData.ctx.storage, 'checkpoint'), params)
    client.userData.ctx.reconfigure()
  }
