    ...
    try {
      if (cmd === 'sidecar') {
        ...
        sp.stdout.on('data', (data) => {
          ...
          for (const line of lines) {
            ...
            if (isHeader && isWindows) {
              ...
              if (!isNaN(id) && ![Bare.pid, pid, sp.pid].includes(id)) {
                const promise = kill(id)
                  .then(result => {
                    stream.push(result)
                    killed.push(id)
                  })
                  .catch(error => {
                    stream.push({ tag: 'error', data: { message: error.message } })
                  })
        ...
        sp.on('exit', async (code, signal) => {
          if (code !== 0 || signal) {
            stream.push({ tag: 'error', data: { message: `Process exited with code: ${code}, signal: ${signal}` } })
          }
          await Promise.all(promises)
          stream.push({ tag: 'complete', data: { killed } })
          stream.push(null)
        })
        ...
        stream.push({ tag: 'error', data: { message: `Unknown command '${cmd}'` } })
      }
    } catch (err) {
