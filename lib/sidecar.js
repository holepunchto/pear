...
class Sidecar {
  ...
  closeClients () {
    ...
    for (const app of this.apps) {
      if (!app.ctx) continue // ignore e.g. `pear sidecar` cli i/o client
      ...
      if (tearingDown === false) app.client.close()
    }
    return metadata
  }
...
class Engine extends ReadyResource {
  ...
  constructor (sidecar, { updater, drive, corestore }) {
    ...
    this.App = class App {
      ...
      constructor ({ client, id = '', startId = '', ctx = null, bundle = null, session }) {
        this.app = this
        this.client = client
  ...
  async * seed ({ name, channel, key, verbose, seeders, dir, clientArgv } = {}, client) {
    ...
    try {
      ...
      ctx.key = key
      client.userData = new this.App({ client, ctx, session })
  ...
  async * stage ({ channel, key, dir, dryRun, name, truncate, bare = false, clientArgv, ignore = '.git,.github,.DS_Store' }, client) {
    ...
    try {
      ...
      await session.add(bundle)
      client.userData = new this.App({ client, ctx, bundle })
  ...
  async #start (session, client, argv, env, cwd, startId) {
    ...
    const app = client.userData = client.userData || new this.App({ client, id, startId, session })
