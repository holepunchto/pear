  ...
  async * gc ({ pid, cmd }, client) {
    ...
    try {
      if (cmd === 'sidecar') {
        ...
        const { killed } = await this.sidecar.gc.sidecar({ pid, name, flag })
...
class Collector {
  async sidecar ({ pid = null, name = 'pear-runtime', flag = '--sidecar' } = {}) {
    const current = Bare.pid
    const ids = await this.#lookupProcess({ name, flag })
    const filtered = ids.filter(id => id !== pid && id !== current)
    const killed = await this.#kill(filtered)
    return { killed }
  }

  async #lookupProcess ({ name = null, flag = null } = {}) {
    if (!name || !flag) throw new Error('Process name or flag are missing')
    ...
    return new Promise((resolve, reject) => {
      ...
      sp.stderr.on('data', data => { reject(new Error(data.toString())) })
  ...
  async #kill (ids) {
    ...
    const args = isWindows ? ['/F', '/PID'].concat(ids) : ids
    ...
    return new Promise((resolve, reject) => {
      ...
      sp.on('exit', (code, signal) => {
        if (!signal && (code === 0 || code === null)) {
          ...
          reject(new Error(`Failed to kill process. Exit code: ${code} ${signal || ''}`))
        }
      })
    })
  }
