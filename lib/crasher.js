...
function setupCrashHandlers (processName, swap, enableLog) {
  ...
  runContext.on('unhandledRejection', (reason) => {
    ...
    enableLog ? logCrashAndExit(crashlogPath, errorInfo, checkout, stack, reason) : printAndExit(errorInfo, stack, reason)
  })

  runContext.on('uncaughtException', (err) => {
    const errorInfo = `${processName} exiting due to uncaught exception`
    enableLog ? logCrashAndExit(crashlogPath, errorInfo, checkout, err.stack, err) : printAndExit(errorInfo, err.stack, err)
  })
}
