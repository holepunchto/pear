...
function logCrash(errorLogPath, errorInfo, stackTrace, err) {
  ...
  appendLog(errorLogPath, errorMsg)

  console.error(`Error logged at ${errorLogPath}`)
}
...
function logAndExit(enableLog, errorLogPath, errorInfo, stack, err) {
  if (enableLog) {
    logCrash(errorLogPath, errorInfo, stack, err)
  } else {
...
function setupCrashHandlers(
  processName,
  enableLog,
  errorLogPath = path.join(PLATFORM_DIR, `${processName}.crash.log`)
) {
  ...
  runContext.on('unhandledRejection', (reason) => {
    ...
    logAndExit(enableLog, errorLogPath, errorInfo, stack, reason)
  })
  ...
  runContext.on('uncaughtException', (err) => {
    ...
    logAndExit(enableLog, errorLogPath, errorInfo, err.stack, err)
  })
}
