...
const EventEmitter = IS_BARE ? require('bare-events') : require('events')
...
const updates = (listener) => {
  messages({ type: 'pear/updates' }, listener)
}
...
class Window extends GuiCtrl { static [kGuiCtrl] = 'window' }
...
function createIpc (config) {
  ...
  if (!ipcRenderer) return null // TODO: convert from electron IPC to jsonrpc-mux, talk directly to sidecar
