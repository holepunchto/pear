...
class API {
  ...
  messages = (pattern, listener) => {
    ...
    const subscriber = this._iterable(`${this.#id}:app:messages`, pattern) // TODO: crank iterable needs to return iambus subscriber
  ...
  restart = async () => this._request({ channel: `${this.#id}:app:restart` })
}
