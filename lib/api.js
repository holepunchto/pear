...
const goodbye = isBare ? require('./teardown') : require('graceful-goodbye')
...
class API {
  ...
  constructor (client, context) {
    ...
    this._ref()
    this._unref()
    this.#unloadingQueue = new Promise((resolve) => { this.#unloading = resolve })
    goodbye(() => this.#unload())
  }
  ...
  async #unload () {
    ...
    await Promise.race([this.#unloadingQueue, timeout])
  }
  ...
  teardown = (fn) => { this.#unloadingQueue = this.#unloadingQueue.then(fn) }
}
