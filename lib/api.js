...
class API {
  ...
  async _ready () {
    ...
    console.log('pre rpc ready')
    await this.#rpc.ready()
    console.log('pre rpc config')
  ...
  constructor (client, ctx, onteardown = teardown) {
    this.#rpc = client
    this.#ctx = ctx
    this.#refs = 0
...
if (global.process?.type === 'renderer') {
  module.exports = require('../gui/api')(API)
} else {
  module.exports = API
}
