...
module.exports = class Interact {
  constructor (header, params, defaults = {}, opts = {}) {
    this._header = header
    this._params = params
    this._defaults = defaults
    this.mask = (data, cb) => {
      if (data.length > 4) { // if length is 4 its backspace
    ...
    this._masked = new Writable({ write: this.mask.bind(this) })
    this._rl = readline.createInterface({
      input: stdio.in,
      output: opts.masked ? this._masked : stdio.out
    })
