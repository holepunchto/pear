...
module.exports = class Interact {
  ...
  async #run (opts = {}) {
    ...
    while (this._params.length) {
      ...
      while (true) {
        ...
        if (answer.length === 0) {
          answer = defaults[param.name] ?? deflt
          stdio.out.write('\n')
        }
  ...
  async #input (prompt) {
    ...
    const empty = new Promise((resolve) => {
      let line = ''
      this._rl._decoder.on('data', (data) => {
        if (line.length === 0 && data.name === 'return') {
          resolve()
        } else {
          line = this._rl.line
        }
    ...
    const answer = (await Promise.race([empty, once(this._rl, 'data')]))?.toString() || ''
