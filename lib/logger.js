...
class Logger {
  ...
  _writeLog(level, label, ...args) {
    if (!this._logPath) return
    const line = formatWithOptions({ colors: false }, level, `[ ${label} ]`, ...args).replace(
      /\u0000/g,
      ''
    )
    saveLog(this._logPath, `${new Date().toISOString()} ${line}\n`)
  }
