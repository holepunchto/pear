...
const { ERR_NO_LINK_SPECIFIED, ERR_INVALID_FLAG } = require('./errors')

const runkey = (link) => {
  if (!link) throw ERR_NO_LINK_SPECIFIED('No link specified')
...
function args (argv, opts = {}) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (validate && !valid.includes(name)) {
      throw ERR_INVALID_FLAG(`Invalid flag: ${flag}`)
    }
