...
module.exports = class State {
  ...
  static configFrom (state) {
    const { id, key, links, alias, env, options, checkpoint, flags, dev, tier, stage, storage, trace, name, main, dependencies, args, channel, release, applink, fragment, link, linkData, entrypoint, dir, dht, bootstrap } = state
    const pearDir = PLATFORM_DIR
    return { id, key, links, alias, env, options, checkpoint, flags, dev, tier, stage, storage, trace, name, main, dependencies, args, channel, release, applink, fragment, link, linkData, entrypoint, dir, dht, bootstrap, pearDir }
  }
  ...
  constructor (params = {}) {
    ...
    this.dht = dht
    this.bootstrap = flags.bootstrap?.split(',').map(e => ({ host: e.split(':')[0], port: Number(e.split(':')[1]) }))
    this.store = store
    this.args = args
