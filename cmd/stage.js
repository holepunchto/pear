...
module.exports = (ipc) => async function stage (cmd) {
  const { dryRun, bare, json, ignore, purge, name, truncate } = cmd.flags
  ...
  const id = Bare.pid
  await output(json, ipc.stage({ id, channel, key, dir, dryRun, bare, ignore, purge, name, truncate, cmdArgs: Bare.argv.slice(1) }), { ask: cmd.flags.ask }, ipc)
}
